...
fourcc=cv2.VideoWriter_fourcc(*'XVID')#compression
...
print(h2,w2)
out=cv2.VideoWriter('cout.avi',fourcc,30.0,(w2,h2))# file name
rout=cv2.VideoWriter('rout.avi',fourcc,30.0,(w2,h2))# file name
bout=cv2.VideoWriter('bout.avi',fourcc,30.0,(w2,h2))# file name
oout1=cv2.VideoWriter('oout.avi',fourcc,30.0,(w2,h2))# file name
...
while(cam.isOpened()):
    ...
    rf=cv2.bitwise_and(rf,rf,mask=rfm) #red
    bf=cv2.bitwise_and(bf,bf,mask=bfm) #blue
    gf=cv2.bitwise_and(gf,gf,mask=gfm)
    co3=cv2.merge((bf,gf,rf)) #color
    ...
    rfc=cv2.merge((rf,rf,rf))
    bfc=cv2.merge((bf,bf,bf))
    ...
    bf2=bf.copy()
    out.write(co3) #color>co3 red>rfc blue>bfc original>color2
    rout.write(rfc) #color>co3 red>rfc blue>bfc original>color2
    bout.write(bfc) #color>co3 red>rfc blue>bfc original>color2
    oout1.write(color2) #color>co3 red>rfc blue>bfc original>color2
    cmd=cv2.waitKey(25)
    if cmd&0xFF == ord('q'):
        break
    if cmd&0xFF == ord('f'):
